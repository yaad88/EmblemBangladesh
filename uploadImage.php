...
if($numOfBlog>0)
{
  ...
  while ($row=$result->fetch_assoc()) {
  	//if($row['Approval']==1)
  	//{
    	$blogList[$index]=array('hname'=>$row['hero_name'],'eid'=>$row['eid']);
  ...
  <body>
   <header>
      ...
      <p class="footer-title">1st image Hero moment, 2nd Hero Face, Erpor Emblem Multiple</p>
					...
					<form action="./controller/PicUpload.php" method="post" enctype="multipart/form-data">
						<div class="row contact_left_grid">
							<div class="col-md-6 con-left">
								<div class="form-group">
                                <label class="my-2">Hero Name</label>
									<select name="hero" type = "text">
									
										<?php

										while ($index<sizeof($blogList)){
											echo
											'<option value="'.$blogList[$index][eid].'" type = "text">'.$blogList[$index][hname].'</option>'.
